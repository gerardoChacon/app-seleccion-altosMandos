...
export default function PerfilEvaluadoPage() {
  ...
  const tieneEvaluacion = aptitudes.length > 0;

  if (!tieneEvaluacion) {
    navigate(`/perfil-sin-evaluacion/${id}`, { replace: true });
    return null;
  }
  ...
  const puntajeTotal = Math.round(
    aptitudes.reduce((sum, a) => sum + Number(a.pivot.porcentaje_obtenido), 0) / aptitudes.length
  );

  const metricas = aptitudes.slice(0, 4).map((a, i) => ({
    label:   a.nombre_aptitud,
    puntaje: Number(a.pivot.porcentaje_obtenido),
          ...
          <div className={styles.rightCard}>
            <div className={styles.resultSection}>
              ...
              <p className={styles.resultLabel}>
                {puntajeTotal >= 90 ? "¡Excelente!" : puntajeTotal >= 75 ? "¡Bien!" : "En desarrollo"}
              </p>
